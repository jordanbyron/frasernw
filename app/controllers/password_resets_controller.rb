  ...
  def create
    if params.blank? || params[:user].blank? || params[:user][:email].blank?
      ...
      if @user
        ...
        @user = User.new
        redirect_to new_password_reset_url, :layout => 'user_sessions', :notice => "We do not have any account associated with #{params[:user][:email]}. Please check the e-mail address you used and try again.   If you continue to encounter problems please contact administration@pathwaysbc.ca"
      end
    end
  end
