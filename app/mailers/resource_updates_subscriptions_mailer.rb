class ResourceUpdatesSubscriptionMailer < ActionMailer::Base
  ...
  def send_subscription_email(activities_for_subscription, tracked_activity_objects, subscription)
    ...
    mail(:to => item.owners.map{|owner| owner.email}, :from => 'noreply@pathwaysbc.ca', :subject => "Pathways: Your #{@interval} Resource Update")
  end
end