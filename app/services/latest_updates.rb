class LatestUpdates
  ...
  def exec
    ...
    Rails.cache.fetch("#{max_automated_events}_#{divisions.map(&:id).sort.join("_")}", force: force) do
      ...
      while automated_events.length <= max_automated_events do
        ...
        versions = Version.
          ...
          offset(offset_multiplier * VERSION_INCREMENT).
          limit(VERSION_INCREMENT)
        break if versions.none?
