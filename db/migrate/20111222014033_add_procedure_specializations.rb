class AddProcedureSpecializations < ActiveRecord::Migration
  def change
    ...
    Procedure.all.each {
      ...
      ps = ProcedureSpecialization.find_by(
        procedure_id: p.id,
        specialization_id: p.specialization_id )
      parent_ps = ProcedureSpecialization.find_by(
        procedure_id: p.parent.id,
        specialization_id: p.specialization_id)
      ps.parent = parent_ps
      ps.save
    }

    # migrate capacities to point at a procedure_specialization, not procedure
    # (as a capacities specialization can be ambigious otherwise)
    Capacity.all.each {
      |c|
    ...
    # migrate focuses to point at a procedure_specialization, not procedure
    # (as a focuses specialization can be ambigious otherwise)
    Focus.all.each {
      |f|
