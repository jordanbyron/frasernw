class BackfillPublicActivity < ActiveRecord::Migration
  def up
    ...
    ScItem.find_each do |item|
      item.create_activity action: :create, update_classification_type: Subscription.resource_update, type_mask: item.type_mask, type_mask_description: item.type, format_type: item.format_type, format_type_description: item.format, parent_id: item.sc_category.root.id, parent_type: item.sc_category.root.name, owner: item.division
